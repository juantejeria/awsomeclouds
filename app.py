...
from core.breed_coefficients import BREED_OPTIONS, CATEGORY_OPTIONS, AGE_OPTIONS, get_estimated_height, HEIGHT_BY_CATEGORY_AGE
...
try:
    ...
    weight_estimator = WeightEstimator(
        ...
        poste1_height_cm=VARA_CM,
        poste2_height_cm=VARA_CM,
...
def modelos_disponibles():
    ...
    for vaca, carpeta in modelo_dirs.items():
        ...
        if peso_real is not None:
            ...
            peso = round(float(vol_barril) * DENSIDAD_KG_L, 1)  # derivar del barril cuando no hay peso_barril guardado
        else:
...
def recalcular_volumen(vaca):
    ...
    peso_kg = round(volumen_litros * DENSIDAD_KG_L, 1)
