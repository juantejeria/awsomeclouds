"""
Agrega los frames de cada individuo de checkpoints/6mayo y checkpoints/"12 junio"
al dataset de barril training (_barril_training), para poder anotar el PUNTO DE
LA CRUZ de esos individuos en el editor (editor_barril_training.py) e incluirlos
luego en el entrenamiento de cruz_pose.

Mismo flujo que agregar_frames_mayo_barril.py:
1. Detecta la vaca (YOLO COCO clase 19)
2. Segmenta el barril con barril_seg.pt
3. Recorta al bbox con padding
4. Guarda img + mask en _barril_training
5. Agrega al indice como 'pending'

Las etiquetas de dataset son '6mayo' y '12junio' (sin espacio) para que el
individuo quede como '6mayo_116_316' / '12junio_000_306', igual que en app.py y
en cruz_resultados.csv.
...
# label de dataset -> carpeta fuente (ojo: "12 junio" lleva espacio en disco)
SOURCES = {
    '6mayo':   PROJECT / 'checkpoints' / '6mayo',
    '12junio': PROJECT / 'checkpoints' / '12 junio',
}
...
def main():
    ...
    for dataset, source_dir in SOURCES.items():
        ...
        for ind_dir in sorted(source_dir.iterdir()):
            ...
            individuo = f"{dataset}_{ind_dir.name}"      # p.ej. 6mayo_116_316
