...
def procesar_foto(img, cow_model, coco_model, cow_height_cm, nombre="foto", seg_model=None, barril_model=None):
    ...
    # Intentar YOLO-seg primero, fallback a GrabCut (silueta completa, para referencia)
    ...
    # Fallback solo si barril_seg no detecta nada
    if mask_barril is None or contorno_barril is None:
        mask_barril = mask_full
        contorno_barril = contorno_full
        barril_method = "silueta-completa"

    print(f"    [{seg_method}+{barril_method}] ", end="")

    # ── Modelo 3D construido desde el BARRIL ──
    pts_b, pts_i = samplear(contorno_barril, mask_barril)
    puntos_px, tris = triangular(pts_b, pts_i, mask_barril)
    ...
    # Volumen del barril (integración por rebanadas sobre la máscara de barril)
    try:
        vol_barril_litros, sup_cm2 = volumen_por_rebanadas(mask_barril, escala)
    except Exception:
        vol_barril_litros = sup_cm2 = 0

    # Volumen silueta completa (para referencia)
    try:
        vol_total_litros, _ = volumen_por_rebanadas(mask_full, escala)
    except Exception:
        vol_total_litros = 0
...
def generar_visualizacion(nombre_vaca, peso_real, mejor, metricas, output_path):
    ...
    ax3.set_title(f'Barril (vol={metricas["vol_barril_litros"]:.0f}L)')
    ax3.axis('off')
    ...
    # 6. Modelo barril = modelo principal (ya es barril)
    ax6 = fig.add_subplot(2, 4, 6)
    ax6.set_facecolor('black')
    if len(tris) > 0:
        polys_b = [puntos_cm[t] for t in tris]
        fcolors_b = [(colores[t] / 255.0).mean(axis=0) for t in tris]
        ax6.add_collection(PolyCollection(polys_b, facecolors=fcolors_b, edgecolors='none', alpha=0.9))
        ax6.set_xlim(puntos_cm[:, 0].min()-3, puntos_cm[:, 0].max()+3)
        ax6.set_ylim(puntos_cm[:, 1].min()-3, puntos_cm[:, 1].max()+3)
    ax6.set_title(f'Barril 3D ({metricas["vol_barril_litros"]:.0f}L \u2192 {metricas["vol_barril_litros"]*1.03:.0f}kg)')
...
BARRIL:
  ...
  Volumen:         {metricas['vol_barril_litros']:.1f} litros
  Peso barril:     {metricas['vol_barril_litros']*1.03:.1f} kg
...
def fusionar_fotos(resultados, cow_height_cm):
    ...
    masks_norm = []
    for r in resultados:
        # Usar mask_barril en vez de mask_full
        mask_key = 'mask_barril' if 'mask_barril' in r else 'mask_full'
        m = normalizar_mask_a_canvas(r[mask_key], r['bbox'])
        if m is not None:
    ...
    contorno_key = 'contorno_barril' if 'contorno_barril' in resultados[0] else 'contorno_full'
    mejor_ref = max(resultados, key=lambda r: cv2.contourArea(r[contorno_key]))
...
BARRIL (solo torso):
  Vol barril:      {metricas['vol_barril_litros']:.1f} litros
...
def procesar_individuo(individuo_dir, fotos_dir, output_dir, cow_model, coco_model, cow_height_override=None, seg_model=None, barril_model=None):
    ...
    for i, foto in enumerate(fotos):
        ...
        r = procesar_foto(img, cow_model, coco_model, cow_height_cm, foto.name, seg_model=seg_model, barril_model=barril_model)
        if r is None:
            continue
        print(f"Vol: {r['vol_total_litros']:.1f}L ({r['vol_total_litros']*1.03:.1f}kg) | Barril: {r['vol_barril_litros']:.1f}L ({r['vol_barril_litros']*1.03:.1f}kg)")
        resultados.append(r)
    ...
    vol_barril_promedio = statistics.mean([r['vol_barril_litros'] for r in validos])
    peso_barril_promedio = vol_barril_promedio * 1.03
    vol_total_promedio = statistics.mean([r['vol_total_litros'] for r in validos])

    # Mejor foto para visualización: la más cercana al promedio de barril
    mejor = min(validos, key=lambda r: abs(r['vol_barril_litros'] - vol_barril_promedio))

    print(f"\n  PROMEDIO ({len(validos)} frames): barril={vol_barril_promedio:.1f}L → {peso_barril_promedio:.1f}kg (real={peso_real}kg, target 50%={peso_real*0.5:.0f}kg)")
    ...
    vols_barril = [r['vol_barril_litros'] for r in validos if r['vol_barril_litros'] > 0]
    vol_barril_mean = statistics.mean(vols_barril) if vols_barril else 0

    contorno_ref = mejor.get('contorno_barril', mejor.get('contorno_full'))
    ...
    metricas = {
        ...
        'vol_barril_litros': round(vol_barril_promedio, 1),
    ...
    resumen = {
        ...
        'vol_barril_litros': round(vol_barril_promedio, 1),
        'peso_barril_kg': round(vol_barril_promedio * 1.03, 1),
        'vol_barril_std': round(statistics.stdev([r['vol_barril_litros'] for r in validos]) if len(validos) > 1 else 0, 1),
        'fotos_procesadas': len(resultados),
        'fotos_validas': len(validos),
        'fotos_descartadas': len(descartados),
        'resultados_por_foto': [
            {
                ...
                'peso_barril_kg': round(r['vol_barril_litros'] * 1.03, 1),
...
def main():
    ...
    output_dir = project / "output_modelos3d_grandes"
    ...
    if alturas_path.exists():
        ...
        print(f"Alturas reales cargadas: {len(alturas)} individuos")
    ...
    if barril_path.exists():
        ...
        print("  cow.pt (detección) + yolov8s-seg.pt (silueta) + barril_seg.pt (barril) cargados")
    else:
        print("  cow.pt (detección) + yolov8s-seg.pt (segmentación) cargados")
        print("  NOTA: barril_seg.pt no encontrado, usando heurística para barril")

    # Buscar individuos que tengan carpeta 3d_modelo_*
    individuos = []
    for ind_dir in sorted(dataset_dir.iterdir()):
        if not ind_dir.is_dir():
            continue
        # Buscar subcarpeta 3d_modelo_* (case insensitive)
        fotos_dir = None
        for sub in ind_dir.iterdir():
            if sub.is_dir() and sub.name.lower().startswith('3d_modelo'):
                # Verificar que tiene fotos
    ...
    for ind_dir, fotos_dir in individuos:
        ...
        resumen = procesar_individuo(ind_dir, fotos_dir, output_dir, cow_model, coco_model, cow_height_override=cow_height, seg_model=seg_model, barril_model=barril_model)
        if resumen:
    ...
    for r in resumen_global:
        ...
        peso_barril = r['vol_barril_litros'] * 1.03
        print(f"  {r['individuo']:<20} {r['peso_real_kg']:>8} kg {r['vol_total_litros']:>8.1f} {peso_vol:>8.1f} kg "
              f"{r['vol_barril_litros']:>10.1f} {peso_barril:>8.1f} kg")
