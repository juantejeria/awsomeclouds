"""Procesa 21 frames + altura conocida → modelo 3D consenso del barril.

Calibración:
- Frame 0 (central, frame_000_*.jpg): altura del bbox del animal = altura_cm
  conocida → cm_per_px GLOBAL.
- Todos los demás frames usan ese mismo cm_per_px (asumimos perspectiva
  estable entre los 21 frames consecutivos).

Consenso ENVELOPE:
- Por columna: MAX(tops) + MIN(bottoms) entre los frames → silueta del barril
  reconstruida ignorando frames con oclusión por poste.

Salida: output_modelos3d_live/<cow_name>/{*_lateral.ply, *_3d.ply}
Volumen reportado: volumen encerrado de la malla cerrada *_3d.ply.

Uso:
    python procesar_21_frames.py <carpeta_frames> <altura_cm> <cow_name>
"""
import os
import sys
import json
...
def _detectar_y_segmentar(img):
    """Devuelve (bbox_yxyx, barril_binmask) o (None, None) si falla."""
...
_output_root = os.environ.get('MODELO_OUTPUT_DIR', 'output_modelos3d_6mayo')
out_dir = proj_dir / _output_root / cow_name
...
resumen = {
    ...
    'metodo': 'envelope_21_frames_carpeta',
