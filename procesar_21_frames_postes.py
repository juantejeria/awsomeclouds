"""Procesa 21 frames usando los POSTES como calibración (no el bbox cow).

Calibración:
- cm_per_px GLOBAL = 50 / mean(tape_px_post1, tape_px_post2) leído desde
  context.json/locked_reference. Constante para todos los frames del video.

Consenso ENVELOPE + variantes A_mediana / B_p75 igual que la versión anterior.

Salida: output_modelos3d_live_postes/<cow_name>/

Uso:
    python procesar_21_frames_postes.py <carpeta_frames> <altura_cm> <cow_name>
"""
import os
import sys
import json
...
# Cargar calibración desde context.json
ctx_path = folder / 'context.json'
if not ctx_path.exists():
    print(f"[error] sin context.json en {folder}"); sys.exit(1)
ctx = json.load(open(ctx_path))
lr = ctx.get('locked_reference')
if not lr:
...
for fp in frame_files:
    ...
    cm_per_px_i = CM_PER_PX_FIJO  # CALIBRADO POR POSTES (constante)
    if binmask is None:
...
resumen = {
    ...
    'metodo': 'envelope_21_frames_calibracion_postes',
    'frames_usados': len(contornos),
    'carpeta_origen': str(folder),
    'cm_per_px_postes': CM_PER_PX_FIJO,
    'tape_avg_px': tape_avg,
