...
function startDetectPassings() {
    ...
    // Prompt 2: frame final (default = fin del video)
    var endStr = prompt('¿Hasta qué frame procesar?\n' +
        'Default = fin del video (' + totalFrames + ').\n' +
        'Arrancamos desde frame ' + startFrame + '.', totalFrames);
    if (endStr === null) {
        stopDetectPassings();
        return;
    }
    var endFrame = parseInt(endStr) || totalFrames;
    if (endFrame > totalFrames) endFrame = totalFrames;
    if (endFrame <= startFrame) {
        alert('El frame final debe ser mayor al inicial (' + startFrame + ').');
        stopDetectPassings();
        return;
    }

    $('#screeningProgressText').text('Detectando pasadas del frame ' + startFrame + ' al ' + endFrame + '…');
    ...
    async function fireRequest(frameNum, blob) {
        ...
        try {
            ...
            if (data && data.success && data.detected) {
                ...
                // Auto-stop: una vez identificado el cruce, paramos cuando
                // tengamos 10 frames con vaca detectada cuyo frameNum sea
                // mayor (es decir, 10 posteriores al cruce). Los frames
                // anteriores que ya estaban en vuelo se completan igual.
                var BARRIL_POST_WINDOW = 10;
                var crossFrame = null;
                for (var ci = 0; ci < AppState.passingResults.length; ci++) {
                    if (AppState.passingResults[ci].cow_height_cm != null) {
                        var fn = AppState.passingResults[ci].frameNum;
                        if (crossFrame == null || fn < crossFrame) crossFrame = fn;
                    }
                }
                if (crossFrame != null) {
                    var postCount = 0;
                    for (var pj = 0; pj < AppState.passingResults.length; pj++) {
                        if (AppState.passingResults[pj].frameNum > crossFrame) postCount++;
                    }
                    if (postCount >= BARRIL_POST_WINDOW) {
                        AppState.passingAbort = true;
                    }
                }
            } else {
    ...
    async function renderPassingThumbnail(blob, data, isValid) {
        ...
        if (oc) {
            ...
            [pL, pR].forEach(function(p) {
                ctx.beginPath();
                ctx.moveTo(p.cx*scale, p.top_tape*scale);
                ctx.lineTo(p.cx*scale, (p.top_tape + p.tape_px)*scale);
                ctx.stroke();
            });
        }
        ...
        if (isValid && data.x_cross !== undefined && data.y_cross !== undefined) {
            ...
            var lbl = Math.round(data.cow_height_cm) + ' cm';
            ctx.font = 'bold 14px sans-serif';
            var tw = ctx.measureText(lbl).width;
            var labelX = crossX + 10;
            var labelY = crossY - 8;
            if (labelX + tw + 10 > W) labelX = crossX - tw - 18;
            ctx.fillStyle = 'rgba(0,0,0,0.85)';
            ctx.fillRect(labelX, labelY - 14, tw + 10, 18);
            ctx.fillStyle = 'rgba(255,193,7,1)';
            ctx.fillText(lbl, labelX + 5, labelY - 1);
        } else {
...
async function generate3DFromResult() {
    ...
    try {
        ...
        // El _volumen.ply que deja acá se sobrescribe en el paso 2.
        var video = document.getElementById('videoPlayer');
        ...
        // ── Paso 2: volumen consenso multi-frame ──
        // Sobrescribe _volumen.ply (malla elipsoidal del consenso) y _resumen.json
        // (con vol_barril_litros = consenso). NO toca _3d.ply ni _lateral.ply.
...
function finalizePassingResults() {
    ...
    if (consensusB != null) {
        ...
        barrilMsg = ' · Barril <strong>(A med):</strong> ' + fmt(cv.A_median) +
            ' · <strong>(B p75):</strong> ' + fmt(cv.B_p75) +
            ' · <strong>(C maxW+p75):</strong> ' + fmt(cv.C_maxW_p75) +
            ' · <strong>(E envelope):</strong> ' + fmt(cv.E_envelope) +
...
async function processFolder() {
    var folder = prompt('Nombre de la carpeta en checkpoints/22abril/ (ej. central160_20260429_110236):');
    if (!folder) return;
    var cowName = prompt('Nombre del modelo (ej. vaca1):');
    if (!cowName) return;
    cowName = cowName.replace(/[^A-Za-z0-9_-]/g, '');
    if (!cowName) { alert('Nombre inválido.'); return; }
    ...
    try {
        ...
        alert('Modelo 3D generado: ' + cowName + '\n' +
              ...
              'Barril (A med): ' + (cv.A_median || '–').toFixed(1) + ' L\n' +
              'Barril (B p75): ' + (cv.B_p75 || '–').toFixed(1) + ' L\n' +
              'Barril (C maxW+p75): ' + (cv.C_maxW_p75 || '–').toFixed(1) + ' L\n' +
              'Barril (E envelope): ' + (cv.E_envelope || '–').toFixed(1) + ' L\n' +
              'Carpeta: ' + folder);
    } catch (e) {
...
function showSelectionPanel(scanData, frameNum) {
    ...
    $('#btnConfirmSelection').off('click').on('click', function() {
        ...
        // Con ref fijada NO mandamos post_indices (backend usa la ref vía video_id)
        if (AppState.lockedReference) {
            AppState.selectedPostIndices = null;
